...
function timelineChart(data){
    ...
    for(let key in data){
        ...
        const startData = d[0];
        const endData = d[d.length-1];
        const startTime = timeSegment(startData.time);
        const endTime = timeSegment(endData.time);
        const start = timelineMapping(startTime,axisHeight,width - padding - axisHeight,padding+axisHeight/2);
        const end = timelineMapping(endTime,axisHeight,width - padding - axisHeight,padding+axisHeight/2);
